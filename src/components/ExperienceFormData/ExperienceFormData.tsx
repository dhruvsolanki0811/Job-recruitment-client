...
function ExperienceFormData({ close,updateData,type }: {type:"ADD"|"EDIT", close: () => void, updateData?:{
  ...
  const [formData, setFormData] = useState({
    ...
    start_month: updateData?.start_month?updateData.start_month:"",
    start_year: updateData?.start_year||"",
    end_month: updateData?.end_month||"",
    end_year: updateData?.end_year||"",
  });
  
  const modalContainer = useRef<HTMLFormElement>(null);
  const {mutate:updateExperienceData,isLoading:isUpdateLoading}=useUpdateExperience()
  const {mutate:addExperience,isLoading:isExperienceLoading}=useAddExperience()
 
  const handleOutsideClick = (e: MouseEvent) => {
    console.log(modalContainer.current?.contains(e.target as Node))
    if (
