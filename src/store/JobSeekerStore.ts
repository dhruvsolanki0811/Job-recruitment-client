...
export const useJobSeekerState = create<JobSeekerState>((set) => ({
  ...
  fetchSingleJobSeeker: async (username) => {
    try {
      ...
      const response = await axios.get(
        `${APIBASEURL}/account/jobseeker/${username}`
      );
  ...
  fetchJobseekers: async () => {
    try {
      ...
      response = await axios.get<JobSeeker[]>(
        `${APIBASEURL}/account/create/jobseeker`
      );
    ...
    try {
      ...
      if (
        ...
        for (const [_, value] of Object.entries(
          ((err as any)?.response?.data ?? {}) as Record<string, unknown>
        )) {
