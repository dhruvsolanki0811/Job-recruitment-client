...
function CompanyDescription() {
  ...
  const { data: organizationPage, isLoading: organizationLoading } =useFetchSingleOrganization(organizationName);

  const { data: jobList, isLoading: jobsLoading } =useFetchOrganizationJobs(organizationName);
          ...
          {organizationLoading || jobsLoading ? (
            ...
            <div className="people-grid flex  p-3  w-full ">
              ...
              <div className="about-sec flex flex-col justify-center mt-2 items-center">
                <div className="header-about text-xs">
                  {jobList && (jobList&&organizationPage) &&
                    jobList.filter((job)=>job.organization_name==organizationPage.name).length > 0 ? "Jobs" : ""}
                </div>
                <div className="people-grid flex p-3  w-full ">
                  {(jobList&&organizationPage) &&
                    jobList.filter((job)=>job.organization_name==organizationPage.name).slice(0, 3).map((job) => (
                      <>
                        <div
                          onClick={() => {
